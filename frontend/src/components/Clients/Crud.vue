...
onMounted(async function(){
    const data = await fetch("http://localhost:8000/create/client")
...
const sendClient = async function(){
    const data = await fetch("http://localhost:8000/send/client", {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
        body: JSON.stringify({
            client_for_user_id: "josueadmin" /* CHANGE IN PRODUCTION FOR A REAL ONE */,
    ...
    <div class="grid grid-cols-[0.2fr_0.8fr] h-screen relative">
        ...
        <div class="p-6 h-screen overflow-auto bg-gray-50">
            <div class="flex justify-between items-center">
                <h1 class="font-bold text-3xl">Clientes - Añadir cliente</h1>
                <div class="flex gap-2 items-center">
                    ...
                    <button class="px-4 py-2 text-sm rounded-md bg-zinc-800 text-white cursor-pointer hover:bg-zinc-600" @click="sendClient">Añadir nuevo</button>
                </div>
            </div>
